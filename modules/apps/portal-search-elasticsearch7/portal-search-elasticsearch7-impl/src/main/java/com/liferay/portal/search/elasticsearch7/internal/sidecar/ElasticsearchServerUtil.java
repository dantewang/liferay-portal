...
public class ElasticsearchServerUtil {
	...
	private static Closeable _enableAppender() throws Exception {
		...
		String appenderName = _LOGGER_NAME + "_capture";

		CaptureAddressAppender captureAddressAppender =
			new CaptureAddressAppender(appenderName);

		captureAddressAppender.start();

		configuration.addAppender(captureAddressAppender);

		LoggerConfig loggerConfig = LoggerConfig.createLogger(
			false, Level.INFO, _LOGGER_NAME, "true",
			new AppenderRef[] {
				AppenderRef.createAppenderRef(appenderName, null, null)
			},
			null, configuration, null);

		loggerConfig.addAppender(captureAddressAppender, null, null);

		configuration.addLogger(_LOGGER_NAME, loggerConfig);
		...
		return () -> {
			...
			configuration.removeLogger(_LOGGER_NAME);
