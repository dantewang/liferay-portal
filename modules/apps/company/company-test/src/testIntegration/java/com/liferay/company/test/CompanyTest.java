...
public class CompanyTest {
	...
	private void _addCompanyAndUser(String webId) throws Exception {
		Company company = _companyLocalService.addCompany(
			null, webId, webId, webId, false, 0, true);

		PortalInstances.initCompany(_mockServletContext, webId);

		_addUser(company.getCompanyId(), webId);
	}

	private void _addUser(long companyId, String webId) throws Exception {
		...
		for (int i = 0; i < _USERCOUNT; i++) {
			...
			_userLocalService.addUser(
				0, companyId, true, null, null, false, screenName, emailAddress,
				LocaleUtil.US, firstName, middleName, lastName, prefixId,
