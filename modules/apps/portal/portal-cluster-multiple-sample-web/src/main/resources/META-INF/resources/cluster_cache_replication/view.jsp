...
<%
PortalCache<String, String> testPortalCache = (PortalCache<String, String>)request.getAttribute(ClusterPortletKeys.PORTAL_CACHE_NAME);
...
<c:if test="<%= keys.isEmpty() %>">
	...
	<span class="redText largeFont">test.cache is empty! </span>
	<br />
</c:if>

<c:if test="<%= !keys.isEmpty() %>">
	<span class="largeFont">Following data is cache get </span>
	<br /><br />

	<liferay-portlet:actionURL name="/cache_removeAll" var="clusterCacheRemoveAllURL" />
	...
	<table class="center">
		...
		<c:forEach items="<%= maps %>" var="map" varStatus="loop">
			<tr class="myBorder">
				...
				<td>
					<aui:button-row>
						<liferay-portlet:actionURL name="/cache_remove" var="clusterCacheRemoveURL">
...
<liferay-portlet:actionURL name="/cache_put" var="clusterCachePutURL"></liferay-portlet:actionURL>
