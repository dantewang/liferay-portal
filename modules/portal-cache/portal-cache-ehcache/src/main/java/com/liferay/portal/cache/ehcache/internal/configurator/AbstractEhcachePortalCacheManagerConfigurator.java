...
public abstract class AbstractEhcachePortalCacheManagerConfigurator {
	...
	protected void resolvePortalProperty(
		...
		if (Validator.isNotNull(propertySeparatorPortalPropertyKey)) {
			String value = props.get(propertySeparatorPortalPropertyKey);

			if (_log.isInfoEnabled()) {
				_log.info(
					"portalPropertyKey " + propertySeparatorPortalPropertyKey +
						" has value " + value);
			}

			factoryConfiguration.setPropertySeparator(value);
		}
