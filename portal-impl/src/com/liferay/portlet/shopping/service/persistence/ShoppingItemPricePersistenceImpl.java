...
public class ShoppingItemPricePersistenceImpl extends BasePersistenceImpl<ShoppingItemPrice>
	...
	public List<ShoppingItemPrice> findByItemId(long itemId, int start,
		...
		List<ShoppingItemPrice> list = (List<ShoppingItemPrice>)FinderCacheUtil.getResult(FINDER_PATH_WITH_PAGINATION_FIND_BY_ITEMID,
				finderArgs, this);
		...
		if (list == null) {
			...
			try {
				...
				if (list == null) {
					FinderCacheUtil.removeResult(FINDER_PATH_WITH_PAGINATION_FIND_BY_ITEMID,
						finderArgs);
				}
				else {
					cacheResult(list);

					FinderCacheUtil.putResult(FINDER_PATH_WITH_PAGINATION_FIND_BY_ITEMID,
						finderArgs, list);
				}
	...
	public ShoppingItemPrice updateImpl(
		...
		if (isNew || !ShoppingItemPriceModelImpl.COLUMN_BITMASK_ENABLED) {
			...
			if ((shoppingItemPriceModelImpl.getColumnBitmask() &
					FINDER_PATH_COUNT_BY_ITEMID.getColumnBitmask()) != 0) {
	...
	public List<ShoppingItemPrice> findAll(int start, int end,
		...
		List<ShoppingItemPrice> list = (List<ShoppingItemPrice>)FinderCacheUtil.getResult(FINDER_PATH_WITH_PAGINATION_FIND_ALL,
				finderArgs, this);
