...
public class TicketPersistenceImpl extends BasePersistenceImpl<Ticket>
	...
	public static final FinderPath FINDER_PATH_FIND_BY_KEY = new FinderPath(TicketModelImpl.ENTITY_CACHE_ENABLED,
			TicketModelImpl.FINDER_CACHE_ENABLED, TicketImpl.class,
			FINDER_CLASS_NAME_LIST, "findByKey",
			new String[] {
	...
	protected List<Ticket> findByKey(String key, int start, int end,
		...
		List<Ticket> list = (List<Ticket>)FinderCacheUtil.getResult(FINDER_PATH_FIND_BY_KEY,
				finderArgs, this);
		...
		if (list == null) {
			...
			try {
				...
				if (list == null) {
					FinderCacheUtil.removeResult(FINDER_PATH_FIND_BY_KEY,
						finderArgs);
				}
				else {
					cacheResult(list);

					FinderCacheUtil.putResult(FINDER_PATH_FIND_BY_KEY,
						finderArgs, list);
				}
	...
	public int countByKey(String key) throws SystemException {
		...
		if (count == null) {
			...
			try {
				...
				if (count == null) {
					FinderCacheUtil.removeResult(FINDER_PATH_COUNT_BY_KEY,
						finderArgs);
				}
				else {
					FinderCacheUtil.putResult(FINDER_PATH_COUNT_BY_KEY,
						finderArgs, count);
				}
